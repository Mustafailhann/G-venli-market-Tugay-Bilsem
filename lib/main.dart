...
class _UrunListesiEkraniState extends State<UrunListesiEkrani> {
  ...
  bool _islemDevamEdiyor = false; // İşlem kilit mekanizması
  
  // Debug ve Status
  String _statusMesaji = 'Başlatılıyor...';
  Color _statusRenk = Colors.orange;

  // ── Tam Ekran Renk Flash Sistemi ──────────────────────────────────────────
  Color _flashRenk = Colors.green;
  double _flashOpacity = 0.0; // Başlangıçta görünmez
  Timer? _flashTimer;
  // ─────────────────────────────────────────────────────────────────────────
  ...
  void initState() {
    ...
    _kartOkuyucuController.addListener(() {
      ...
      if (_odemeDialogAcik && metin.length >= 10) {
        // ✅ Kilit kontrolü: aktif işlem varsa kart okumasını yoksay
        if (_islemDevamEdiyor) {
          print('🔒 İşlem devam ediyor, kart okuması reddedildi (dialog mod)');
      ...
      if (metin.length >= 10) {
        // ✅ Kilit kontrolü: aktif işlem varsa yeni kart okumasını tamamen yoksay
        if (_islemDevamEdiyor) {
          print('🔒 İşlem devam ediyor, kart okuması reddedildi: $metin');
          _kartOkuyucuController.clear();
          return;
        }
        ...
        _debounceTimer = Timer(const Duration(milliseconds: 200), () {
          ...
          if (mounted && metin.isNotEmpty && !_islemDevamEdiyor) {
            _kartOkutuldu(metin);
            _kartOkuyucuController.clear();
          }
        });
  ...
  void dispose() {
    ...
    _flashTimer?.cancel();    // Flash zamanlayıcısını temizle
  ...
  // ── Tam Ekran Renk Flash ─────────────────────────────────────────────────
  /// Ekranın üstüne [renk] renginde yarı-saydam bir katman göster.
  /// Katman [gorunmeSuresi] kadar tam opaklıkta durur, ardından yavaşça
  /// kaybolur (AnimatedOpacity ile).
  void _ekranFlash(Color renk, {Duration gorunmeSuresi = const Duration(milliseconds: 2200)}) {
    if (!mounted) return;
    _flashTimer?.cancel();

    // 1. Katmanı anında görünür yap
    setState(() {
      _flashRenk = renk;
      _flashOpacity = 0.55;
    });

    // 2. [gorunmeSuresi] sonra yavaşça söndür (AnimatedOpacity duration: 800ms)
    _flashTimer = Timer(gorunmeSuresi, () {
      if (!mounted) return;
      setState(() {
        _flashOpacity = 0.0;
      });
  ...
  Future<void> _kartIsleminiYap(String kartID) async {
    // ── İşlem Kilidi: çift harcama korunası ──
    if (_islemDevamEdiyor) {
      print('🔒 _kartIsleminiYap reddedildi: önceki işlem henüz bitmedi.');
      return;
    }
    _islemDevamEdiyor = true; // KİLİDİ KAPAT
    ...
    if (ogrenci == null) {
      print("❌ Öğrenci bulunamadı: $temizKartID");
      if (mounted) _sonucGoster("Tanımsız Kart", "Bu kart sisteme kayıtlı değil.\nKart ID: $temizKartID", false, false);
      _islemDevamEdiyor = false; // KİLİDİ AÇ (hata)
      return;
    }
    ...
    if (toplamTutar == 0) {
      ...
      _islemDevamEdiyor = false; // KİLİDİ AÇ (bakiye sorgu)
      return;
    }
    ...
    // Limit kontrolü
    final double limit = (ogrenci.tip == 'Personel') ? -50.0 : -10.0;
    if ((ogrenci.bakiye - toplamTutar) < limit) {
      print("❌ Yetersiz Bakiye!");
      if (mounted) {
        _ekranFlash(Colors.red);
        _sonucGoster(
          "Yetersiz Bakiye",
    ...
    // ── GÜVENLİ ÖDEME: Başarı mesajı yalnızca Firebase onayından sonra gösterilir ──
    bool odemeBasarili = false;
    String hataMesaji = '';

    try {
      odemeBasarili = await veriYoneticisi
          .odemeYap(temizKartID, toplamTutar, sepet, islemFotografiYolu: fotoYolu)
          .timeout(const Duration(seconds: 15));
    } on TimeoutException {
      hataMesaji = 'Bağlantı zaman aşımına uğradı.\nİşlem kaydedilemedi. İnternet bağlantınızı kontrol edin.';
      print("⏱️ Ödeme timeout: Firebase 15 saniyede yanıt vermedi");
    } on FirebaseException catch (e) {
      hataMesaji = 'Sunucu hatası oluştu.\nHata kodu: ${e.code}';
      print("🔥 Firebase hatası: ${e.code} - ${e.message}");
    } catch (e) {
      if (e.toString().contains('Yetersiz Stok')) {
        hataMesaji = 'Yetersiz stok!\nSeçtiğiniz üründen yeterli miktar bulunmuyor.';
      } else {
    ...
    if (odemeBasarili) {
      // ✅ SADECE Firebase onayı alındıktan sonra başarı göster
      final guncelOgrenci = veriYoneticisi.ogrenciBul(temizKartID);
      final kalanBakiye = guncelOgrenci?.bakiye ?? (ogrenci.bakiye - toplamTutar);
      print("✅ Ödeme başarılı ve Firebase'e kaydedildi. Kalan: $kalanBakiye TL");
      _ekranFlash(Colors.green); // 🟢 YEŞİL FLASH
      _sonucGoster(
        "✅ Ödeme Başarılı!",
      ...
      // ❌ Firebase'e yazılamadı — kullanıcıya açık hata mesajı ver
      print("❌ Ödeme başarısız veya Firebase'e kaydedilemedi");
      _ekranFlash(Colors.red); // 🔴 KIRMIZI FLASH
      _sonucGoster(
        "❌ Ödeme Başarısız",
        hataMesaji.isNotEmpty
            ? hataMesaji
            : "İşlem tamamlanamadı.\nLütfen tekrar deneyin veya kantinle iletişime geçin.",
        false,
        false,
      );
    }

    // ── Flash animasyonu + diyalog kapatılınca kilidi serbest bırak ──
    // Flash toplam süresi: 2200ms görünme + 900ms solma = ~3100ms
    // Diyalog'un da kapanmış olması için 3300ms bekle
    Future.delayed(const Duration(milliseconds: 3300), () {
      if (mounted) _islemDevamEdiyor = false;
      print('🔓 İşlem kilidi serbest bırakıldı. Yeni kart okuması kabul edilebilir.');
    });
  }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        children: [
          ...
          // ── TAM EKRAN RENK FLASH KATMANI ──────────────────────────────────
          // Her zaman Stack'in en üstünde. AnimatedOpacity sayesinde
          // yavaşça belirir ve yavaşça solar — arkadaki içerik sürekli görünür.
          AnimatedOpacity(
            opacity: _flashOpacity,
            duration: _flashOpacity > 0.0
                ? const Duration(milliseconds: 120)  // Belirme: çok hızlı
                : const Duration(milliseconds: 900), // Solma: yavaş ve akıcı
            curve: Curves.easeOut,
            child: IgnorePointer( // Flash sırasında dokunma engellenmesin
              child: Container(
                color: _flashRenk.withOpacity(1.0), // opacity AnimatedOpacity'den geliyor
                child: Center(
                  child: AnimatedOpacity(
                    opacity: _flashOpacity > 0.0 ? 1.0 : 0.0,
                    duration: const Duration(milliseconds: 150),
                    child: Icon(
                      _flashRenk == Colors.green ? Icons.check_circle_outline : Icons.cancel_outlined,
                      color: Colors.white,
                      size: 160,
                    ),
                  ),
